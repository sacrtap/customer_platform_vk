...
沉淀可复用经验（经 `.agents/skills/trellis-update-spec/` 或 `learn`）时，每条记录必须包含两段可追踪信息，供后续同类任务对照验证：

1. **基线（Baseline）**：本次要解决的问题是什么——现象、根因、之前的错误做法或缺失的约定（一行即可，具体可核验）。
2. **预期改进（Expected Improvement）**：沉淀后应该发生什么变化——下次同类任务不再犯同一错误 / 不再重复排查同一问题 / 按新约定执行。

**对照验证**：遇到同类任务时，先对照既有沉淀判断"基线问题是否不再出现"；结论（有效 / 部分有效 / 无效）记录在 `.trellis/workspace/` 日志或任务笔记中。无效的沉淀应更新或废弃，避免陈旧约定长期滞留。
