...
test.describe('用户管理增强测试', () => {
  ...
  test('L01: PageHeader — eyebrow "System" + 标题 "用户管理"', async ({
  ...
  test('L03: 创建用户', async ({ authenticatedPage: page }) => {
    ...
    // 填入用户名
    const usernameInput = modal
      .locator('input[field="username"], .arco-form-item:has-text("用户名") input')
      .first()
    ...
    const emailInput = modal
      .locator('input[placeholder*="邮箱"], .arco-form-item:has-text("邮箱") input')
      .first()
  ...
  test('L05: 重置密码弹窗', async ({ authenticatedPage: page }) => {
    ...
    const resetBtn = page.locator(
      '.arco-table button:has-text("重置密码"), .arco-table button:has-text("重置")'
    )
    const hasData = await resetBtn
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false)
    ...
    if (inputCount >= 2) {
      ...
      if (
        ...
        // 验证不一致提示
        await expect(page.locator('.arco-form-item-message:has-text("不一致")')).toBeVisible({
          timeout: 5000,
        })
      }
    }
  ...
  test('L06: 删除用户 — 确认弹窗', async ({ authenticatedPage: page }) => {
    ...
    const popconfirm = page.locator(
      '.arco-popconfirm:visible, .arco-popover-popup:visible, .arco-modal:visible'
    )
    ...
    if (await emailInput.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (
        ...
        const errorMsg = page.locator(
          '.arco-form-item-message:has-text("邮箱"), .arco-form-item-message:has-text("格式")'
        )
