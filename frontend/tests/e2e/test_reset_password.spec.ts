...
test.describe('重置密码页面', () => {
  test('D01: 无 token 访问显示"重置链接无效"', async ({ page }) => {
    ...
    await expect(page.locator('.reset-container, .reset-box'))
  ...
  test('D03: 密码不一致校验', async ({ page }) => {
    ...
    await expect(errorMessage.filter({ hasText: /不一致|不同/ }).first()).toBeVisible({
      timeout: 5000,
    })
  })
  ...
  test('D04: 密码长度校验', async ({ page }) => {
    ...
    await expect(page.locator('.arco-form-item-message:has-text("6")')).toBeVisible({
      timeout: 5000,
    })
  })
