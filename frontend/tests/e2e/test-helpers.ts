...
export async function apiRecharge(
  token: string,
  customerId: number,
  realAmount: number = 1000,
  bonusAmount: number = 0
): Promise<Record<string, unknown>> {
  const { status, data: response } = await apiRequest(
    'POST',
    `${TEST_CONFIG.apiBase}/billing/recharge`,
    {
...
export async function apiUpdateCustomer(
  ...
  const { status, data: response } = await apiRequest(
    'PUT',
    `${TEST_CONFIG.apiBase}/customers/${customerId}`,
    data,
    { Authorization: `Bearer ${token}` }
  )
...
export async function apiCreateTag(
  ...
  const { status, data: response } = await apiRequest('POST', `${TEST_CONFIG.apiBase}/tags`, data, {
    Authorization: `Bearer ${token}`,
  })

  if (status !== 200 && status !== 201) {
    throw new Error(`API 创建标签失败: ${status} - ${JSON.stringify(response)}`)
  }

  return response
}

/** 通过后端 API 创建/更新客户画像（规模等级、消费等级等） */
export async function apiUpdateCustomerProfile(
  ...
  const { status, data: response } = await apiRequest(
    'PUT',
    `${TEST_CONFIG.apiBase}/customers/${customerId}/profile`,
    data,
    { Authorization: `Bearer ${token}` }
  )

  if (status !== 200 && status !== 201) {
    throw new Error(`API 更新客户画像失败: ${status} - ${JSON.stringify(response)}`)
  }

  return response
}

/** 通过后端 API 生成结算单（默认生成 draft 状态，用于需要草稿结算单的用例前置准备） */
export async function apiGenerateInvoice(
  ...
  const { status, data: response } = await apiRequest(
    'POST',
    `${TEST_CONFIG.apiBase}/billing/invoices/generate`,
    data,
    { Authorization: `Bearer ${token}` }
  )

  if (status !== 200 && status !== 201) {
    throw new Error(`API 生成结算单失败: ${status} - ${JSON.stringify(response)}`)
  }
...
export async function uiLogin(page: Page): Promise<void> {
  // 快速路径：global-setup 已通过 storageState 注入登录态（省去每用例 2-3 秒登录开销）。
  // 注意用 localStorage 中的 token 判断，而非 URL：goto('/') 后的重定向是异步的，
  // 此刻 page.url() 可能尚未变化，据此判断会误跳过登录。
  await page.goto('/', { waitUntil: 'domcontentloaded', timeout: 30000 })
  const hasToken = await page.evaluate(() => Boolean(localStorage.getItem('access_token')))

  if (!hasToken) {
    // 回退路径：storageState 缺失或失效时执行完整登录（保持原有「确保已登录」语义）
...
export async function fillFormField(page: Page, label: string, value: string): Promise<void> {
  // 优先在可见弹窗中查找表单项
  const modalFormItem = page.locator('.arco-modal:visible .arco-form-item', {
...
export async function waitForTableLoaded(page: Page, timeout: number = 15000): Promise<void> {
  // 等待加载动画消失（Arco 表格）
  await page
    .waitForSelector('.arco-table .arco-spin-loading', {
      state: 'hidden',
      timeout,
    })
    .catch(() => {})
  ...
  await page
    .waitForSelector('.arco-table tbody, .table-section tbody, table.table tbody', { timeout })
    .catch(() => {})
...
export async function expectTableRowContains(
  ...
  const matchingRows = page.locator(
    '.arco-table tbody tr, .table-section tbody tr, table tbody tr',
    { hasText: text }
  )
...
export async function clearFilters(page: Page): Promise<void> {
  ...
  const accountTypeTrigger = page
    .locator('.filter-dropdown', { hasText: '账号类型' })
  ...
  const industryTrigger = page
    .locator('.filter-dropdown', { hasText: '行业' })
    .locator('.filter-trigger')
...
export async function searchCustomer(page: Page, keyword: string): Promise<void> {
  ...
  const searchInput = page
    .locator('.filters-container input[placeholder*="搜索"], .filters input[placeholder*="搜索"]')
    .first()
  ...
  await page
    .locator('.filters-container button:has-text("筛选"), .filters-actions button:has-text("筛选")')
    .first()
    .click()
  await waitForTableLoaded(page)
}
