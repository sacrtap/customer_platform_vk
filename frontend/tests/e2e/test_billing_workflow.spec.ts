...
test.describe('结算单管理工作流 @smoke', () => {
  test.beforeAll(async () => {
    // 「状态流转」用例依赖列表中已存在草稿状态的结算单，缺数据时只能 skip；
    // 在此固定创建一个客户并为其生成一张草稿结算单，保证该用例确定性执行。
    // 生成可能因缺少计费规则而失败——失败时保留原 skip 兜底，不使套件变红。
    const token = await apiLogin()
    const created = await apiCreateCustomer(token, {
      company_id: generateTestCompanyId(),
      name: generateTestCustomerName('结算单单据'),
      account_type: '正式账号',
    ...
    if (customerId) {
      await apiGenerateInvoice(token, {
        customer_id: customerId as number,
        period_start: '2026-03-01',
        period_end: '2026-03-31',
        items: [{ device_type: 'X', layer_type: 'single', quantity: 100, unit_price: 10 }],
      }).catch(() => {})
    }
  })
  ...
  test('结算单状态流转 - 提交 → 运营确认 → 销售确认 → 客户确认', async ({
