...
test.describe('客户管理 CRUD @smoke', () => {
  ...
  test('1. 访问客户列表页面', async ({ page }) => {
    ...
    // 验证操作按钮存在
    const _headerActions = page.locator(
      '.arco-page-header-actions, .header-actions, [class*="header"] button'
    )
    const hasCreateBtn = await page
      .locator('button:has-text("新增客户")')
      .first()
  ...
  test('2. 创建新客户 - 基本信息', async ({ page }) => {
    ...
    const accountTypeSelect = page
      .locator('.arco-modal:visible .arco-form-item', { has: page.locator('text=账号类型') })
      .locator('.arco-select')
      .first()
    ...
    const industrySelect = page
      .locator('.arco-modal:visible .arco-form-item', { has: page.locator('text=行业类型') })
      .locator('.arco-select')
      .first()
  ...
  test('4. 编辑客户信息', async ({ page }) => {
    ...
    const found = (updated.data?.list || updated.data?.items || []).some(
      (c: { name: string }) => c.name === newName
    )
  ...
  test('5. 删除客户', async ({ page }) => {
    ...
    await searchCustomer(page, String(companyId))
    await page.waitForTimeout(1000)
    const remainingRows = page.locator('table tbody tr, .table-section tbody tr', {
      hasText: String(companyId),
    })
  ...
  test('6. 取消删除操作', async ({ page }) => {
    ...
    // 通过 API 验证客户存在
    const customers = await apiGetCustomers(authToken, { keyword: String(companyId) })
    const found = (customers.data?.list || customers.data?.items || []).some(
      (c: { company_id: number }) => c.company_id === companyId
    )
    expect(found).toBeTruthy()

    // 不删除客户，验证客户仍然存在
    const customers2 = await apiGetCustomers(authToken, { keyword: String(companyId) })
    const stillExists = (customers2.data?.list || customers2.data?.items || []).some(
      (c: { company_id: number }) => c.company_id === companyId
    )
  ...
  test('8. 创建客户 - 邮箱格式验证', async ({ page }) => {
    ...
    if (inputCount >= 3) {
      ...
      const errorElements = page.locator(
        '.arco-form-item-message-error, .arco-form-item-status-error'
      )
