...
test.describe('数据同步功能', () => {
  test.describe('UI 交互测试', () => {
    test('打开同步对话框 - 显示默认状态', async ({ authenticatedPage: page }) => {
      ...
      const startButton = modal
        ...
        .getByRole('button', { name: /提交同步任务|确定/ })
  ...
  test.describe('同步流程测试（使用 API Mock）', () => {
    test('创建同步任务 - 成功后显示提示', async ({ authenticatedPage: page }) => {
      ...
      const successModal = page
        ...
        .locator('.arco-modal-title', { hasText: /已提交|成功/ })
  ...
  // 说明（2026-09-21）：此处原有 6 个 `test.skip(...)` 占位用例（最小化进度框、完成/失败/取消状态、
  // 进度百分比动态更新）。经核前端源码，SyncDialog 组件并不提供内联进度轮询与最小化能力
  // （创建任务后直接关闭弹窗并提示成功），即这些用例**没有可测对象**，故移除。
  // 保留此说明而非保留永久跳过的空用例：后者会在报告中呈现为 skipped，形成「有覆盖」的错觉。
  // 若将来实现了同步进度轮询/最小化，请在此处补回对应用例。
})
