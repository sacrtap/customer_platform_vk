...
import { getVisibleModal, apiLogin, apiCreateTag, generateTestCustomerName } from './test-helpers'
...
test.describe('标签管理增强测试', () => {
  test.beforeAll(async () => {
    // M04（编辑）/M05（删除）依赖「标签云中存在标签」，缺数据时只能 skip；
    // 在此固定准备一个客户标签，保证这两条用例确定性执行
    const token = await apiLogin()
    await apiCreateTag(token, {
      name: generateTestCustomerName('E2E标签'),
      type: 'customer',
      category: '测试',
    }).catch(() => {})
  })
  ...
  test('M01: PageHeader — eyebrow "System" + 标题 "标签管理"', async ({
  ...
  test('M05: 删除标签 — 关闭标签 → 确认删除', async ({ authenticatedPage: page }) => {
    ...
    const closeBtns = page.locator(
      '.tag-list .arco-tag .arco-icon-close, .tag-list .arco-tag-close'
    )
