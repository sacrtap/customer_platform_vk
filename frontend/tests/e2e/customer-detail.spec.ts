...
test.describe('客户详情页面', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // 补充画像数据：画像 Tab 在无规模/消费等级时会 skip（用例 6/7/8/14 的原跳过原因）
    await apiUpdateCustomerProfile(authToken, testCustomerId, {
      scale_level: 'A',
      consume_level: 'C1',
    }).catch(() => {})
  ...
  test('3. 显示客户标签', async ({ page }) => {
    ...
    const containerVisible = await tagsContainer
      .first()
    ...
    const btnVisible = await addTagBtn
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false)
  ...
  test('5. 移除客户标签', async ({ page }) => {
    ...
    const closeVisible = await tagCloseBtn
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false)
