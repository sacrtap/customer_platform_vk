...
test.describe('布局框架', () => {
  test.describe('Dashboard Grid 布局', () => {
    ...
    test('C01: .shell 使用 grid 布局，grid-template-columns = 189px 1fr', async ({
    ...
    test('C02: 折叠态 .shell.collapsed grid-template-columns = 72px 1fr', async ({
      authenticatedPage: page,
    }) => {
      // 点击折叠按钮
  ...
  test.describe('侧边栏交互', () => {
    ...
    test('C06: 折叠后 localStorage prototype-sidebar-collapsed = true', async ({
      authenticatedPage: page,
    }) => {
      const toggleBtn = page.locator('.toggle-btn')
      await toggleBtn.click()
  ...
  test.describe('顶栏交互', () => {
    ...
    test('C12: 搜索框 role="search" + aria-label="全局搜索"', async ({
      authenticatedPage: page,
    }) => {
    ...
    test('C14: 搜索框 Enter 触发搜索提示', async ({ authenticatedPage: page }) => {
      ...
      const message = page.locator(
        '.arco-message-info, .arco-message-warning, .arco-message-success'
      )
