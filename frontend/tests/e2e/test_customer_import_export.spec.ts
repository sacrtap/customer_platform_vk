...
test.describe('客户导入/导出', () => {
  ...
  test('2. 验证导入弹窗UI元素', async ({ page }) => {
    ...
    // 等待下载模板按钮出现
    await page
      .locator('.arco-modal:visible button:has-text("下载模板")')
      .first()
      .waitFor({ state: 'visible', timeout: 10000 })

    // 验证下载模板按钮可见
    await expect(
      page.locator('.arco-modal:visible button:has-text("下载模板")').first()
    ).toBeVisible()
  ...
  test('3. 上传文件功能可用', async ({ page }) => {
    ...
    try {
      ...
      if (inputExists) {
        ...
        const fileSelected = page.locator(
          '.arco-modal:visible .file-selected, .arco-modal:visible .file-name'
        )
  ...
  test('4. 导入重复数据流程', async ({ page }) => {
    ...
    // 验证弹窗包含导入相关元素
    const hasDownloadTemplate = await page
