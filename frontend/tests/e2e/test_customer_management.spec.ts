...
test.describe('客户管理', () => {
  ...
  test('访问客户列表页面', async ({ authenticatedPage }) => {
    ...
    await expect(
      authenticatedPage.locator('.filters-container input[placeholder*="搜索"]').first()
    ).toBeVisible()
    // 重构后筛选按钮文本为"筛选"
    await expect(
      authenticatedPage
    ...
    await expect(
      authenticatedPage.locator('.table-section table, table.table').first()
    ).toBeVisible()
  })
  ...
  test('创建新客户', async ({ authenticatedPage }) => {
    ...
    let errorCount = 0
    try {
      await authenticatedPage.waitForSelector('.arco-message-success, .arco-message-error', {
        timeout: 8000,
      })
    ...
    if (stillVisible) {
      await authenticatedPage
        .locator('.arco-modal:visible button:has-text("取消")')
        .first()
        .click()
        .catch(() => {})
    }
  ...
  test('搜索客户', async ({ authenticatedPage }) => {
    ...
    const searchInput = authenticatedPage
      .locator('.filters-container input[placeholder*="搜索"]')
      .first()
  ...
  test('编辑客户信息', async ({ authenticatedPage }) => {
    ...
    const nameInput = authenticatedPage.locator(
      '.arco-modal:visible input[placeholder="请输入客户名称"]'
    )
    ...
    const settlementSelect = authenticatedPage.locator(
      '.arco-modal:visible .arco-form-item:has-text("结算方式") .arco-select'
    )
    const hasSettlement = await settlementSelect
      .first()
      .isVisible({ timeout: 2000 })
      .catch(() => false)
    if (hasSettlement) {
      const settlementText = await settlementSelect
    ...
    try {
      ...
      hasSuccess = await authenticatedPage
        .locator('.arco-message-success')
        .first()
        .isVisible()
        .catch(() => false)
      hasError = await authenticatedPage
