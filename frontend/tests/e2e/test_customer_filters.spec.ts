...
test.describe('客户筛选功能 @smoke', () => {
  ...
  // 记录 beforeEach 实际创建的名字：用例若各自再调 generateTestCustomerName()，
  // 会因 Date.now() 不同而生成另一个名字，导致按名字搜索必然落空（原 skip 的直接原因）
  let kaStandardName = ''
  let skaName = ''
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // 创建多组测试数据
    kaStandardName = generateTestCustomerName('KA正式')
    skaName = generateTestCustomerName('SKA测试')
    const customers = [
      {
        company_id: generateTestCompanyId(),
        name: kaStandardName,
        account_type: '正式账号',
      ...
      {
        company_id: generateTestCompanyId(),
        name: generateTestCustomerName('普通测试'),
        account_type: '客户测试账号',
      ...
      {
        company_id: generateTestCompanyId(),
        name: generateTestCustomerName('房产KA'),
        account_type: '正式账号',
        is_key_customer: true,
        settlement_type: 'prepaid',
        industry_type_id: 2,
  ...
  async function clickFilterDropdown(
  ...
  async function clickSearchButton(page: import('@playwright/test').Page): Promise<void> {
    await page
      .locator(
        '.filters-container button:has-text("筛选"), .filters-actions button:has-text("筛选")'
      )
      .first()
      .click({ force: true })
    await waitForTableLoaded(page)
  ...
  test('1. 关键词搜索 - 公司 ID', async ({ page }) => {
    const resp = await apiGetCustomers(authToken, { keyword: kaStandardName })
    const target = (resp.data?.list || resp.data?.items || []).find(
      (c: { name: string }) => c.name === kaStandardName
    )
    ...
    const rows = page.locator('.table-section tbody tr, table tbody tr', {
      hasText: String(target.company_id),
    })
    await expect(rows.first()).toBeVisible({ timeout: 10000 })
  })

  test('2. 关键词搜索 - 客户名称', async ({ page }) => {
    const resp = await apiGetCustomers(authToken, { keyword: skaName })
    const target = (resp.data?.list || resp.data?.items || []).find(
      (c: { name: string }) => c.name === skaName
    )
    test.skip(!target, '无测试数据')

    await searchCustomer(page, target.name)

    // 用 company_id 定位结果行：客户名称在表格中可能被截断，按完整名称匹配会落空
    const rows = page.locator('.table-section tbody tr, table tbody tr', {
      hasText: String(target.company_id),
    })
    await expect(rows.first()).toBeVisible({ timeout: 10000 })
  ...
  test('10. 空结果提示', async ({ page }) => {
    ...
    if (rowCount > 0 && !emptyVisible) {
      const matchingRows = page.locator('.table-section tbody tr, table tbody tr', {
        hasText: 'NOT_EXIST_999999_ABCDEFG',
      })
