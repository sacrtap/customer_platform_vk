...
test.describe('客户管理边界/异常场景', () => {
  ...
  test('1. 创建客户 - 超长客户名称', async ({ page }) => {
    ...
    const errorVisible = await errorMsg
      ...
      .catch(() => false)
    const formErrorVisible = await formError
      .first()
      .isVisible({ timeout: 2000 })
      .catch(() => false)
  ...
  test('4. 快速连续创建（防抖/竞态）', async ({ page }) => {
    ...
    const hasModalContent = await modalBtn
      .first()
  ...
  test('6. 网络错误恢复', async ({ page }) => {
    ...
    // 验证错误处理
    const errorMsg = page.locator('.arco-message-error')
    const _errorVisible = await errorMsg
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false)
  ...
  test('7. 详情页 - 不存在的客户 ID', async ({ page }) => {
    ...
    const errorVisible = await errorMsg
      ...
      .isVisible({ timeout: 3000 })
      .catch(() => false)
    const notFoundVisible = await notFound
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false)
  ...
  test('8. 空数据列表状态', async ({ page }) => {
    ...
    const searchInput = page
      .locator(
        'input[placeholder*="关键词"], input[placeholder*="搜索"], .arco-input-wrapper input'
      )
      .first()
    ...
    const matchingRows = page.locator('.table-section tbody tr, table tbody tr', {
      hasText: 'NONEXISTENT_EDGE_TEST_999999',
    })
  ...
  test('9. 会话超时处理', async ({ page }) => {
    ...
    const hasAuthData = storageKeys.some(
      (k) => k.includes('token') || k.includes('auth') || k.includes('user')
    )
