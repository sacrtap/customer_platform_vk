  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
      ...
      <a-form-item field="attachment" label="附件（选填）">
        ...
        <div v-if="currentAttachment" class="current-attachment">
          <a :href="currentAttachment" target="_blank">当前附件</a>
        </div>
      </a-form-item>
    </a-form>
  </a-modal>
...
import { applyDiscount, uploadDiscountAttachment } from '@/api/billing'
...
watch(
  ...
  (val) => {
    if (val) {
      // 重置所有表单状态
      form.discount_amount = props.currentDiscount || undefined
      form.reason = props.currentReason || ''
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await applyDiscount(props.invoiceId, {
      discount_amount: form.discount_amount || 0,
      discount_reason: form.reason || '',
      discount_attachment: attachmentPath,
    })
    emit('success')
    Message.success('减免信息已更新')
    return true
  } catch (error) {
    handleError(error)
    return false
