<template>
  <div class="erp-systems-page">
    <PageHeader eyebrow="System" title="ERP 系统" subtitle="管理系统 ERP 系统字典">
      <template #actions>
        <a-button v-if="can('erp_systems:manage')" type="primary" @click="handleCreate"
          >新增 ERP 系统</a-button
        >
      </template>
    </PageHeader>
      ...
      >
        <template #action="{ record }">
          <a-space>
            ...
            <a-popconfirm
              v-if="can('erp_systems:manage')"
              content="确认删除该 ERP 系统？删除后不会影响已关联的客户记录。"
        ...
        <template #empty>
          <EmptyState
            title="暂无 ERP 系统数据"
            description="点击「新增 ERP 系统」添加第一个 ERP 系统"
          >
            <template #action>
              <a-button v-if="can('erp_systems:manage')" type="primary" @click="handleCreate"
                >新增 ERP 系统</a-button
    ...
    <a-modal
      ...
      :title="isEditMode ? '编辑 ERP 系统' : '新增 ERP 系统'"
    ...
    >
      <a-form ref="formRef" :model="form" :rules="formRules" layout="vertical">
        <a-form-item field="name" label="ERP 系统名称">
          <a-input v-model="form.name" placeholder="请输入展示名称，如「ERP云」" />
        </a-form-item>
        <a-form-item field="value" label="存储值">
          <a-input v-model="form.value" placeholder="请输入存储值，如「erp_cloud」" />
        </a-form-item>
...
const columns = [
  ...
  { title: 'ERP 系统名称', dataIndex: 'name', width: 200 },
...
const formRules = {
  name: [{ required: true, message: '请输入 ERP 系统名称' }],
...
const loadErpSystems = async () => {
  ...
  try {
    ...
    pagination.total = erpSystems.value.length
  } catch (error) {
    Message.error('加载 ERP 系统失败')
...
const handleEdit = (record: ErpSystem) => {
...
const handleSubmit = async () => {
  try {
    ...
    if (isEditMode.value && editingId.value !== null) {
      await updateErpSystem(editingId.value, {
        name: form.name,
        value: form.value,
...
const handleDelete = async (id: number) => {
  try {
    await deleteErpSystem(id)
    Message.success('删除成功')
    await loadErpSystems()
  } catch (error) {
    Message.error('删除失败')
    console.error('Failed to delete ERP system:', error)
  }
}
