...
## Corpus Check
- 128 files · ~142,197 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1510 nodes · 2496 edges · 75 communities detected
- Extraction: 68% EXTRACTED · 32% INFERRED · 0% AMBIGUOUS · INFERRED: 807 edges (avg confidence: 0.5)
...
- `TestCleanupWebhookSignatures` --uses--> `WebhookSignature`  [INFERRED]
  backend/tests/unit/test_webhook_cleanup.py → backend/app/models/webhooks.py
- `Webhook Cleanup Tasks 单元测试 测试覆盖率目标：80%+` --uses--> `WebhookSignature`  [INFERRED]
  backend/tests/unit/test_webhook_cleanup.py → backend/app/models/webhooks.py
...
Cohesion: 0.02
Nodes (63): check_balance_warning(), _log_check_task(), P6-4: 余额预警检查任务 每小时检查客户余额，标记预警状态, 检查客户余额预警      执行时间：每小时     职责：检查所有客户余额，标记预警状态, BaseModel, 通用 Redis 缓存服务  提供热点数据缓存功能，支持： - 客户列表缓存 - 标签列表缓存 - 客户详情缓存 - 缓存失效管理, SoftDeleteMixin, TimestampMixin (+55 more)

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (85): 获取客户健康度统计（优化：6次查询 → 2次查询）, 获取长期未消耗客户列表（优化：使用子查询替代 Python 集合操作）, 获取仪表盘统计数据（优化：6次查询 → 2次查询）, BalanceService, ConsumptionRecord, CustomerBalance, Invoice, InvoiceItem (+77 more)
...
Cohesion: 0.1
Nodes (2): Webhook Cleanup Tasks 单元测试 测试覆盖率目标：80%+, TestCleanupWebhookSignatures
...
- **Why does `Customer` connect `Community 0` to `Community 8`, `Community 1`, `Community 2`, `Community 19`?**
  _High betweenness centrality (0.117) - this node is a cross-community bridge._
- **Why does `BaseModel` connect `Community 1` to `Community 8`, `Community 0`, `Community 2`, `Community 3`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `CacheService` connect `Community 4` to `Community 1`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Are the 87 inferred relationships involving `Customer` (e.g. with `P6-3: 月度结算单自动生成任务 为非重点客户自动生成上月结算单` and `自动生成月度结算单      执行时间：每月 1 日 02:00     职责：为非重点客户自动生成上月结算单`) actually correct?**
  _`Customer` has 87 INFERRED edges - model-reasoned connections that need verification._
