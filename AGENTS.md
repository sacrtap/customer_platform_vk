...
本项目使用 graphify 构建知识图谱 (`graphify-out/`)。

**图谱位置**:
- `graphify-out/graph.json` - 原始图谱数据
- `graphify-out/graph.html` - 交互式可视化（浏览器打开）
- `graphify-out/GRAPH_REPORT.md` - 审计报告（God Nodes、Surprising Connections）

**自动更新**:
- Git commit 后自动重建图谱（post-commit hook）
- 代码变更无需手动更新

**查询命令**:
```bash
/graphify query "<问题>"           # BFS 遍历查找相关节点
/graphify path "<A>" "<B>"         # 查找两个概念之间的路径
/graphify explain "<概念>"         # 解释某个节点的连接关系
/graphify --update                 # 手动更新图谱
```
...
| 知识图谱 | `graphify-out/GRAPH_REPORT.md` `graphify-out/graph.html` |
