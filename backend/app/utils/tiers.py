...
def _validate_tier_coverage(tiers: List[dict]) -> None:
    ...
    注：首档 ``min`` 允许非 0（前端编辑器要求为 0）。``_calc_tiered`` 按
    ``min(剩余用量, 档位跨度)`` 逐档切块，**不减去首档 ``min`` 偏移**，实测行为：

    - 用量 ≤ 首档 ``max`` 时，``min`` 取值不影响金额（``min=1`` 与 ``min=0`` 等价）；
    - 用量 > 首档 ``max`` 时，档位跨度整体前移 ``min - 1`` 个单位，多出的用量落到
      下一档价格上 → **少收** ``(min - 1) × (首档价 - 次档价)``
      （实测 ``[5..100 @10, 101..∞ @8]`` 用量 100：得 992，按「低于 min 的用量仍按首档价」
      口径应为 1000，少收 8）；
