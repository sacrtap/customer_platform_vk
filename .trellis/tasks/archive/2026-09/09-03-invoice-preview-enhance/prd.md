...
- [ ] AC1: 打开弹窗 → 选择客户和结算周期 → 关闭弹窗 → 再次打开弹窗，结算周期保留显示，客户输入框为空
- [ ] AC2: 打开弹窗 → 结算周期已有值 → 选择客户后自动刷新计费明细预览（无需手动改周期）
- [ ] AC3: 已选客户 → 修改结算周期 → 预览自动刷新
- [ ] AC4: 客户为定价结算 → 预览表格显示"定价"计费类型列、单价、小计
- [ ] AC5: 客户为阶梯结算 → 预览表格显示"阶梯"计费类型列、阶梯配置明细、平均单价、小计
- [ ] AC6: 客户为包年不限量 → 预览表格显示"包年-不限量"、年费/日费/周期天数/周期费
- [ ] AC7: 客户为包年限量 → 预览表格显示"包年-限量"、套餐内用量费+超量费明细
- [ ] AC8: 客户有多条不同设备/楼层的定价规则 → 预览表格显示多行，每行对应一条规则
- [ ] AC9: `calculate-items` API 返回数据包含所有计费规则相关字段
- [ ] AC10: 批量模式下结算周期也保留
