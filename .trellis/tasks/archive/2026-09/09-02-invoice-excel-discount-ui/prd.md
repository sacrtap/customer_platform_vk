...
- [x] AC1: 创建结算单后，异步生成 Excel 明细文件，完成后可通过详情页下载
- [x] AC1b: 列表页和详情页显示明细文件的生成状态（pending/generating/completed/failed）
- [x] AC1c: 系统工具侧边栏有「结算单日志」入口，可查看所有生成任务状态
- [x] AC1d: 明细文件生成失败后，可通过详情页重试按钮或「结算单日志」页面手动触发重新生成
- [x] AC2: 下载的 Excel 包含两个 Sheet（合计+明细），格式与模板一致
- [x] AC3: 明细 Sheet 数据来源于外部数据库 `nest_model_order`，字段映射正确
- [x] AC4: 提交结算单时可填写减免金额（允许负值）、减免说明、附件
- [x] AC5: 最终结算金额 = 总金额 - 减免金额（正负值均正确计算）
- [x] AC6: 详情页显示"减免金额"和"最终结算金额"，不再出现"折扣"/"折后"字样
- [x] AC7: 列表页同步更新标签文字
