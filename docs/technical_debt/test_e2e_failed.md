...
| 2026-09-21 | **E2E 工作流现状对齐**：仓库中不存在 `e2e-nightly.yml`（现行全量 E2E 为 `e2e-full.yml`，仅 `workflow_dispatch` 手动触发、无 cron 调度）；同步修正 `pr-checks.yml` 中指向 `e2e-nightly.yml` 的悬空引用，改为如实描述手动触发路径。TD-012 非阻塞决定保持不变。 | better-harness fix |
