...
**创建日期**: 2026-06-29  
**最后更新**: 2026-07-05 (TD-002 fully resolved)
...
| 文件路径 | 行数 | 问题 |
|---------|------|------|
| `backend/app/routes/billing.py` | 1,913 | 结算路由过于集中 |
| `backend/app/services/analytics.py` | 1,301 | 分析服务逻辑复杂 |
| `backend/app/services/billing.py` | 1,007 | 结算业务逻辑臃肿 |
| `backend/app/routes/customers.py` | 949 | 客户路由功能过多 |
| `backend/app/services/customers.py` | 927 | 客户服务需要拆分 |

**当前核实**: 2026-07-05 仍成立。执行时复核：上述 5 个后端文件仍有 3 个超过 1000 行（billing.py=1913, analytics.py=1301, services/billing.py=1007）、2 个接近 1000 行（customers routes=949, customers services=927），债务仍成立。
...
| 文件路径 | 行数 | 问题 |
|---------|------|------|
| `frontend/src/views/customers/Detail.vue` | 1,217 | 仍超阈值，需继续拆分 |
| `frontend/src/views/customers/Index.vue` | 1,038 | 仍超阈值，需继续拆分 |
| `frontend/src/views/Dashboard.vue` | 191 | 已拆分完成 |
| `frontend/src/views/billing/Balance.vue` | 143 | 已拆分完成 |
| `frontend/src/views/billing/Invoices.vue` | 171 | 已拆分完成 |

**当前核实**: 2026-07-05 **已解决**。実行後検証：

| 文件路径 | 原行数 | 当前行数 | 状态 |
|---------|--------|---------|------|
| `frontend/src/views/customers/Detail.vue` | 2,331 | **163** | ✅ 已拆分 |
| `frontend/src/views/customers/Index.vue` | 2,026 | **175** | ✅ 已拆分 |
| `frontend/src/views/Dashboard.vue` | 1,689 | 191 | ✅ 已拆分 |
| `frontend/src/views/billing/Balance.vue` | 1,373 | 143 | ✅ 已拆分 |
| `frontend/src/views/billing/Invoices.vue` | 1,037 | 171 | ✅ 已拆分 |

已创建 customers/components/（4个子组件）和 billing/components/（9个子组件）。Detail.vue 和 Index.vue 仍需继续拆分。
...
| 日期 | 变更内容 | 操作人 |
|------|---------|--------|
| 2026-07-05 | **TD-002 部分缓解**：Dashboard.vue(1689→191)、Balance.vue(1373→143)、Invoices.vue(1037→171) 已完成拆分；Detail.vue(2331→1217) 和 Index.vue(2026→1038) 仍超阈值需继续；已创建 customers/components/(4个) 和 billing/components/(9个)。**TD-001 更新行数**：billing.py=1913, analytics.py=1301, services/billing.py=1007 仍超阈值 | 代码审查 |
