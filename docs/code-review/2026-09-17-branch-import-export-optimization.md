...
2. **`cost_calc.py:414-415` 首档 `min > 0` 的计费语义**：实测 `min=5/price=10`、用量 3 得 **30**（`min` 未参与计算）；多档形态（`[5..100 @10, 101..∞ @8]`、用量 100）得 **992**，按「低于 `min` 的用量仍按首档价」口径应为 1000 —— 净额**少收 8**，故本项原表述「静默多收」**方向相反**，已更正。**未改的原因**：结算侧修正会改变每日消耗与结算金额（需先定业务口径、并确认是否重算历史账期）；导入侧拒绝只防新增、治不了存量。三个方案的**具体代码与影响**见 **§13.2**，建议 **B + C**。属低危且影响面小，**需你明确选择口径后再落地**。
...
### 12.4 待产品拍板（两条，均为「口径」而非「实现」）—— **已于 2026-09-17 会话处置，详见 §十三**
...
1. **`analytics.py:2302` 平板定价客户的用量维度**：单档无界形态没有「预期用量」参照，任何取值都是产品决策。选项：
   ...
   → **用户已选 B 并落地**。注意：字面「近 30 天日均×30」与旧回退**数学等价**（空操作），
   实际实现取「**前 90 天（第 31~120 天）** 日均 × 30」，详见 **§13.1**。
2. **`cost_calc` 首档 `min > 0` 的计费口径**（§11.4 第 2 条）：结算侧修正（改历史金额）/ 导入侧拒绝（只防新不治旧）/ 维持现状。

   → **已实测澄清并给出具体代码与影响**（§13.2），建议 **B + C**，**待用户确认后方可落地**。
...
## 十三、决策落地：健康度基线（方案 B）与首档 `min > 0` 待确认方案

### 13.1 方案 B 已落地 —— 健康度「用量达标率」改用客户自身历史基线

**缺陷（§12.4 第 1 条）**：模板推荐的统一定价写法 `[{"min":0,"max":null,"price":5}]` 既无末档
`max` 也无非 0 的入口 `min`，`expected_usage` 恒为 0 → 落入回退分支
`expected_usage = actual_usage` → 达标率恒为 100%，占健康度 50% 权重的用量维度完全失效。

**实现前的关键澄清**：原表述「改用近 30 天日均 × 30」**在数学上等价于旧回退** ——
`actual_usage` 本身就是近 30 天用量合计，故 `actual_usage / 30 * 30 == actual_usage`。
照字面实现等于**空操作**（旧代码注释「回退：用近30天的日均 * 30 作为预期」与实现早已不一致，
注释描述的就是这个恒等式）。要让用量维度重新产生信号，参照物必须取自**更早的历史窗口**。

**实现**（`backend/app/services/analytics.py`）：

| 位置 | 内容 |
|---|---|
| `:2259-2260` | 新增基线窗口下限 `one_twenty_days_ago = utcnow() - 120d` |
| `:2305-2331` | 回退分支改为查询「第 31~120 天」的 `SUM(order_count)` 与 `MIN(consumption_date)`，得 `expected_usage = 基线合计 / 覆盖天数 × 30` |

设计要点：

- **窗口刻意排除最近 30 天**：用量上涨时达标率封顶 100%（不误伤），用量回落时才低于 100%。
- **按覆盖天数折算**：开户不足 90 天的客户按窗口内实际天数（`min(90, 覆盖天数)`）折算，
  避免预期用量被系统性低估而导致达标率虚高。
- **无历史数据时保持 0**：`expected_usage` 仍为 0 → `_calculate_usage_rate` 返回 0.0，
  与既有「无数据」语义一致（不改动 `test_get_health_score_no_data` 的断言值）。
- **末档无上界且 `min > 0` 的形态不受影响**：该形态仍取入口边界 `min` 作参照（第 2 轮 M1 决策，
  `test_get_health_score_unbounded_last_tier` 断言 `usage_rate == 49.9` 保持不变）。

**影响面**：所有此前落入回退分支的客户（无阶梯配置 / 单档无界且 `min=0`，即「平板定价」）的
`usage_rate` 与 `score`。平衡性指标（`balance_rate` 30%、`payment_rate` 20%）不变。

**证据**：

```
# 回归防护有效性：临时还原旧回退逻辑后，两个新用例均失败于 100.0（即恒满分的缺陷本身）
pytest -k "flat_pricing or baseline_scales"（pre-fix）
    assert 100.0 == 50.0     # 平板定价 + 用量腰斩：旧实现 100%，正确 50%
    assert 100.0 == 51.67    # 覆盖天数折算：旧实现 100%，正确 51.67%

# 修复后
pytest tests/test_analytics_service.py -k "health_score"    -> 5 passed
```

### 13.2 首档 `min > 0` 的实测澄清与三个候选方案（**待确认，未落代码**）

#### 实测事实（`CostCalcService._calc_tiered` 与 `InvoiceService._calculate_tiered_price` 两份实现结果完全一致）

| 形态 | 用量 | 结果 | 澄清 |
|---|---|---|---|
| `[{min:5, max:null, price:10}]` | 3 / 10 / 100 | 30 / 100 / 1000 | `min` **完全不参与计算**，恒等于「用量 × 价格」（等价平价） |
| `[{min:5,max:100,@10}, {101..∞,@8}]` | 100 | 992（等价 `min=0` 写法应为 1000） | **少收 8** = `(min-1) × (首档价 - 次档价)` |
| 同上 | 200 | 1792（应为 1800） | 少收恒定 8 |
| `[{min:1,max:100,@10}, {101..∞,@8}]` | 101 / 150 | 1008 / 1400（应 1010 / 1402） | `min=1` 与 `min=0` **仅在用量 ≤ 首档 max 时等价** |

> **两处需更正**：
> 1. §11.4/R3-L13 的「静默**多收**」**方向相反** —— 多档形态下档位跨度整体前移 `min-1` 个单位，
>    多出的用量落到**更便宜**的下一档，净额是**少收**；只有单档无界形态下「档位起点」语义失效
>    （无论是否低于 `min` 都按同一单价计费）。
> 2. §12 报告中「改用近 30 天日均 × 30」的表述不成立（见 §13.1）。
>
> `app/utils/tiers.py` 的 `_validate_tier_coverage` docstring 已按本节实测数据改正（第 2 轮
> M12 写入的「首档 `min=1` 与 `min=0` 计费等价」不完整；「静默多收」改为「档位起点失效 /
> 超档后少收」）。**计费行为未改。**
...
```python
# cost_calc._calc_tiered / billing._calculate_tiered_price 同步改（两处算法必须保持等价）
 for index, tier in enumerate(sorted_tiers):
     if remaining <= 0:
         break
-    min_qty = tier.get("min", 0)
+    min_qty = 0 if index == 0 else tier.get("min", 0)   # A1：首档视为从 0 起，低于 min 的用量按首档价
```
- A1（档位起点归零）：上表中 `min=5` 的用例金额 `992 → 1000`，**少收被修掉**。
- A2（低于 `min` 的用量不计费）：`remaining` 需再减去 `(min - 1)`，`min=5`/用量 3 得 **0**，
  `min=5`/用量 100 得 960 —— 需**先定口径**（哪一条才符合业务）。
- **影响**：改变**未发生**的每日消耗与结算金额；已生成的结算单与 `DailyConsumption` 历史记录
  不会自动重算，但**同一账期重新结算会得到不同金额**（对账口径变化）。存量脏规则需盘点。
- **风险**：高（金额敏感 + 口径未定 + 需与业务确认是否需要重算/补差）。

**方案 B — 写入侧拒绝首档 `min ≠ 0`（只防新）**

```python
# app/utils/tiers.py:_validate_tier_coverage 追加（导入路径自动生效）
+if tiers[0]["min"] != 0:
+    raise TierFormatError(
+        f"首档阶梯 min 必须为 0（当前 {tiers[0]['min']}），"
+        "首档起点非 0 时计费语义不明确（见 _calc_tiered 说明）"
+    )
```
- 同时需覆盖规则编辑器写入路径（`PricingService.create_pricing_rule` / `update_pricing_rule`
  的两处 `normalize_tiers` 调用点，当前**不走**覆盖完整性校验）。
- **影响**：新建/编辑/导入的非 0 首档规则被拒绝并给出明确文案；前端编辑器本就强制 `min=0`
  （`PricingRuleModal`），**无 UX 回归**；模板示例行已是 `[{"min":0,...}]`。
